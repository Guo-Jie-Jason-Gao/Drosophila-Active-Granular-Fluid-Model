      ...
      ! The code is written by Dr. G.-J. J. Gao and compiled and tested 
      ! using Intel Fortran Compiler.
      !
      ! (Contact information)
      ! Department of Mathematical and Systems Engineering
      ! Shizuoka University, JAPAN
      ! Email: gao@shizuoka.ac.jp
      !
      ...
      ! (left) in the program   ~   (right) in the paper
      ...
      write(3,*) '# (1) shrink_idx'
      write(3,*) '# (2) step_count (3) tot_V'
      write(3,*) '# (4) #_ran_trial (5) #_unconstricted_affected_part'
      write(3,*) '# (6) ave unconstricted affected (ua) deform'
      write(3,*) '# (7) max unconstricted affected (ua) deform'
      write(3,*) '# (8) ave constricted affected (ca) deform'
      write(3,*) '# (9) max constricted affected (ca) deform'
      write(3,*) '# (10) ave unconstricted unaffected (uu) deform'
      write(3,*) '# (11) max unconstricted unaffected (uu) deform'
      write(3,*) '# (12) ave constricted unaffected (cu) deform'
      write(3,*) '# (13) max constricted unaffected (cu) deform'
 ...
 20   format(5(e21.14,3X),2(I5,3X))
      ...
      if(movie_switch .eq. 1) then
        ...
        write(2,*) nspart
        do i=1,npart
          ! x,y,vx,vy,radius,bdry_label,status_label
...
30      format(I5,3X,I10,3X,e21.14,3X,I10,3X,I5,3X,8(e21.14,3X))

        if(shrink_idx .gt. 0) then
          write(3,30) shrink_idx,saved_step_count,saved_V,
     &                saved_num_trial,
